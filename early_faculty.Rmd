...
faculty <- read_excel('data/Early Career Fac May2022.xlsx',  sheet = 'Sheet1') %>% 
  clean_names() %>% 
...
faculty %>% 
  ...
  annotate("text", label = "Today", size = 3,
           x = ymd(Sys.Date())-105, 
           y = nrow(faculty)/2-2, color = "red") +
  annotate("text", label = "Write/Submit Grants", 
           x = ymd(Sys.Date()) + 365, size = 3,
           y = nrow(faculty)/2-2, 
           color = "blue") +
  annotate("text", label = "2 years from now", 
           x = ymd(Sys.Date()) + 950, size = 3,
           y = nrow(faculty)/2-2, color = "black") +
  geom_rect(xmin = ymd(Sys.Date())+ 1480, 
            xmax = ymd(Sys.Date())+ 2130, , 
            ymin = 15, ymax = 22, 
            color = "blue", fill = "white") +
  annotate("text", label = "4 Faculty Transitioned \nto Independent Funding", size = 4,
           x = ymd(Sys.Date())+ 1805, 
           y = nrow(faculty)/2-18.5, color = "black") +
